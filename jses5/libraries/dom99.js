...
var DOM99 = function () {
  "use strict"; //it must start with "data-" see
  ...
  var walkTheDom = function walkTheDom(node, aFunction) {
    aFunction(node);
  ...
  var addEventListener = function addEventListener(node, type, aFunction, useCapture) {
  ...
  var onceAddEventListener = function onceAddEventListener(node, type, aFunction, useCapture) {
    if (useCapture === void 0) {
      useCapture = false;
    }

    var tempFunction = function tempFunction(event) {
      //called once only
      aFunction(event);
  ...
  var tryExecuteData99Directive = function tryExecuteData99Directive(node, prefix, execution) {
    if (node.hasAttribute(prefix) && node.getAttribute(prefix) !== dom99AfterValue) {
      execution(node, node.getAttribute(prefix).split("-")); //ensure the directive is only executed once
  ...
  var linkJsAndDom = function linkJsAndDom(startNode) {
    if (startNode === void 0) {
